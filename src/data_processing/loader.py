...
def _clean_json_strings(node: Any) -> Any:
    """
    Recursively walks a (json) node and sanitizes all strings
    to remove non-UTF-8 characters, preventing crashes.
    """
    if isinstance(node, dict):
        ...
        for key, value in list(node.items()):
            if isinstance(value, str):
                ...
                # Encode to bytes (ignoring bad chars)
                # and decode back to a clean utf-8 string.
                node[key] = value.encode('utf-8', errors='ignore').decode('utf-8')
            else:
                # Recurse into nested dicts/lists
                _clean_json_strings(value)
    elif isinstance(node, list):
        for item in node:
            _clean_json_strings(item)
