...
class PictureDescriptionMarkdownSerializer(MarkdownPictureSerializer):
    END_TAG = "<!-- image description ends -->"
    ...
    def __init__(self, add_placeholder_for_missing=False, drop_images_without_description=True):
        ...
        self._img_desc_idx = -1 

    def _next_idx(self) -> int:
        self._img_desc_idx += 1
        return self._img_desc_idx
    ...
    def serialize(
        ...
        **kwargs: Any,
        ...
        if not desc:
            idx = self._next_idx()
            if self.drop_images_without_description:
                return create_ser_result(text="", span_source=item)

            if self.add_placeholder_for_missing:
                desc = "Imagem sem descrição gerada (possível elemento decorativo ou recorte inválido)."
            else:
                # Keep whatever Docling produced for the picture, but don't add description tags
                return parent_res

        # We will output a description block => increment counter
        idx = self._next_idx()
        start_tag = f"<!-- image {idx} description -->"
        end_tag = self.END_TAG

        # Try to detect the placeholder configured in MarkdownParams (fallback to your old one)
        placeholder = "<!-- image -->"
        try:
            placeholder = (
                getattr(getattr(doc_serializer, "params", None), "image_placeholder", None)
                or placeholder
            )
        except Exception:
            pass
        ...
        # Put the start tag where the placeholder was (if present). Otherwise, add it explicitly.
        if parent_res.text:
            if placeholder in parent_res.text:
                parts.append(parent_res.text.replace(placeholder, start_tag))
            else:
                # e.g. if parent_res.text is an actual markdown image reference
                parts.append(parent_res.text)
                parts.append(start_tag)
        else:
            parts.append(start_tag)

        # Add description (avoid duplicating if somehow already embedded)
        if desc and desc not in (parent_res.text or ""):
            parts.append(desc)
...
def _dedupe_image_descriptions(md: str) -> str:
    """
    Removes duplication in the pattern:
      <desc>\n\n<!-- image N description -->\n\n<same desc>\n\n<!-- image description ends -->
    Keeps only:
      <!-- image N description -->\n\n<desc>\n\n<!-- image description ends -->

    Assumes the description is a single paragraph (no internal newlines), which matches _clean_picture_desc().
    """
    pattern = re.compile(
        r"(?P<desc>[^\n][^\n]*)\n{2,}"
        r"<!-- image (?P<idx>\d+) description -->\n{2,}"
        r"(?P=desc)(?=\n{2,}<!-- image description ends -->)"
    )

    prev = None
    while md != prev:
        prev = md
        md = pattern.sub(lambda m: f"<!-- image {m.group('idx')} description -->\n\n{m.group('desc')}", md)

    return md


def parse_pdf_docling_images(
    pdf_path: str,
    output_dir: str,
    ...
    try:
        ...
        md_text = _dedupe_image_descriptions(md_text)
