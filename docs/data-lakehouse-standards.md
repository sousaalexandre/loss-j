...
**Version:** 1 | **Date:** 22 December 2025 | **Authors:** Professor Rui Pereira, Ricardo Yang
...
**Scope:** The specification defines mandatory standards for the **Landing Zone** (input interface) and **Gold Layer** (output interface), while permitting flexibility in internal processing layers (Bronze, Silver) to accommodate diverse pipeline implementations. This document serves as both a reference implementation and a governance framework for maintaining data quality and consistency across ETL operations.
...
* **Layer 0: Landing Zone** (`00_landing_zone`) - Immutable raw input.
* **Layer 1: Bronze** (`01_bronze`) - Raw extraction (Internal).
* **Layer 2: Silver** (`02_silver`) - Curated and enriched (Internal).
* **Layer 3: Gold** (`03_gold`) - Consumption-ready output.

> **Note:** Layers 1 (Bronze) and 2 (Silver) do not have a fixed structure as they are internal processing layers. Their schema and organization are customized based on pipeline-specific requirements and may vary between implementations. Only the Landing Zone and Gold Layer interfaces are standardized for consistency across all pipelines.


## 2. Layer 0: Landing Zone (`00_landing_zone`)
The Landing Zone is the immutable entry point for all source documents. It uses **Content-Addressable Storage (CAS)** to ensure data integrity and deduplication.
...
### 2.3 The Landing Catalog (`_catalog.json`)
...
### 3.3 The Gold Catalog (`_catalog.json`)
Located at `03_gold/_catalog.json`, this registry mirrors the Landing Zone catalog but includes processing metrics.
